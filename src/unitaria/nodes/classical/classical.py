...
class Classical(Node):
    ...
    def _subspace_in(self) -> Subspace:
        return Subspace(bits=self.input_bits, zero_qubits=max(0, self.output_bits - self.input_bits))

    def _subspace_out(self) -> Subspace:
        return Subspace(bits=self.output_bits, zero_qubits=max(0, self.input_bits - self.output_bits))
