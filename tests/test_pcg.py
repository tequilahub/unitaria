...
class LCG(ProxyNode):
    ...
    def __init__(self,
                 bits: int,
                 constant_add: int | None = None,
                 constant_mul: int | None = None):
        self.bits = bits
    ...
    def definition(self) -> Node:
        ...
        for i in range(self.bits):
            ...
            step = Mul(ConstantIntegerMultiplication(self.bits, acc_mul),
                       ConstantIntegerAddition(self.bits, acc_add),
                       skip_projection=True)
            controlled = (Identity(Subspace([ID] * self.bits + [ZeroQubit()] * 2 + [ID] * i)) |
                          (step & Identity(i))) & Identity(self.bits - i - 1)
            if result is None:
                result = controlled
            else:
                # TODO: skip_projection should be done automatically here
