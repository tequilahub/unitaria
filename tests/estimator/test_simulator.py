...
def test_monte_carlo():
    ...
    for precision in [0.1, 0.01]:
        ...
        for i in range(2, 4):
            ...
            new_complexity = simulator.complexity
            assert (new_complexity - old_complexity) > 2 * n * (node.normalization / precision) ** 2
...
def test_phase_estimation():
    ...
    for precision in [0.1, 0.01]:
        ...
        for i in range(2, 4):
            ...
            assert np.abs(simulator.estimate_norm(node[:n]) - np.linalg.norm(v[:n])) < precision
            new_complexity = simulator.complexity
            assert (new_complexity - old_complexity) > 2 * n * (node.normalization / precision)
            old_complexity = new_complexity
